...
void Chat::runServer()
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            //_threads.push_back(std::thread(&Chat::participantHandler, iConnection - 1));
            participantHandler(iConnection - 1);
        }
    }
}
...
{
    std::string login = getParticipantString(i);
    std::string password = getParticipantString(i);
    std::string name = getParticipantString(i);
...
void Chat::addParticipant(int id, std::string login, std::string password, std::string name)
{
...
void Chat::addParticipantToDatabase(int id, std::string login, std::string password, std::string name)
{
    //string str;
...
void Chat::authorization(int i)
{
    ...
    {
        sendParticipantInt(i, -1);
        sendParticipantString(i, "NoneName");
        return;
    }
...
bool Chat::signIn(std::string login, std::string password)
{
...
uint Chat::findHash(std::string password)
{
...
{
    std::string nameSender = getParticipantString(i);
    std::string nameRecipient = getParticipantString(i);
    std::string text = getParticipantString(i);
...
void Chat::sendMessage(std::string nameSender, std::string nameRecipient, std::string text)
{
    ...
    for (size_t i = 0; i < _participants.size(); ++i)
        if (nameRecipient == _participants[i].getName())
        {
            ...
            str.append("INSERT INTO messages(id, id_recipient, id_sender, text) values(default, ")
                .append(std::to_string(idRecipient)).append(", ").append(std::to_string(idSender)).append(", '")
                .append(text).append("')");
            //mysql_query(&mysql, str.c_str());
...
void Chat::recordMessage(std::string nameSender, std::string nameRecipient, std::string text)
{
    Message message;
...
int Chat::countSentMessages(std::string nameSender)
{
    int count = 0;

    for (size_t m = 0; m < _messages.size(); ++m)
        if (nameSender == _messages[m].getSender())
            count++;

    return count;
}

int Chat::countReceivedMessages(std::string nameRecipient)
{
    int count = 0;
...
bool Chat::compareLogin(std::string login)
{
    for (size_t i = 0; i < _participants.size(); ++i)
    {
        if (login == _participants[i].getLogin())
...
bool Chat::compareName(std::string name)
{
    if (name == "all")
        return true;
...
std::string Chat::getParticipantName(std::string login)
{
    for (size_t i = 0; i < _participants.size(); ++i)
        if (login == _participants[i].getLogin())
...
int Chat::getParticipantIndex(std::string name)
{
    for (size_t i = 0; i < _participants.size(); ++i)
        if (name == _participants[i].getName())
            return i;
...
std::string Chat::getParticipantString(int i)
{
    int sizeWord;
...
void Chat::sendParticipantString(int i, std::string word)
{
